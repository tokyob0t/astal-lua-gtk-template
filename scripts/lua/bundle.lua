...
__modules['src.bundle'] = function(...)
    ...
    local function log(str)
        if settings.verbose then print(str) end
    end
    local function process_file(f)
        ...
        for line in f:lines() do
            ...
            write(' ')
            while i <= #line do
                local function skip_to_end_of_line() i = #line + 1 end
                ...
                local function scan_string()
                    ...
                    while char_at(line, i) ~= search do
                        if char_at(line, i) == '\\' then i = i + 1 end
                        i = i + 1
                    end
                    i = i + 1
                ...
                if search_depth > 0 then
                    if line:sub(i, i + #searching_for - 1) == searching_for then
                        ...
                        i = i + #searching_for
                        if preserve_original_content then write(searching_for) end
                    elseif line:sub(i, i + #search_start - 1) == search_start then
                        search_depth = search_depth + 1
                        i = i + #search_start
                        if preserve_original_content then write(search_start) end
        ...
        if explore then
            for _, i in ipairs(require_indices) do
                ...
                local function skip_whitespace()
                    while char_at(contents, i) == ' ' or char_at(contents, i) == '\t' do
                        i = i + 1
                    end
                end
                if i <= #contents then
                    ...
                    if char_at(contents, i) == '"' or char_at(contents, i) == "'" then
                        ...
                        while char_at(contents, i) ~= search do
                            if char_at(contents, i) == '\\' then i = i + 1 end
                            i = i + 1
                        end
                        local path
                        ...
                        if valid then table.insert(requires, path) end
                    end
                end
            end
        end
        if not settings.minify then contents = original_contents end
        return { minified = contents, requires = requires }
    end
        ...
        for _, module in ipairs(requires) do
            if not modules[module] and not array_contains(settings.ignore_modules, module) then
                ...
                if file then
                    ...
                    file:close()
                    modules[module] = { content = processed.minified, path = full_path }
                    explore_requires(processed.requires)
                else
                    local warning = "module '" .. module .. "' not found"
                    if not settings.suppress_warnings then log('WARNING: ' .. warning) end
    ...
    local function bundle(entry_file, s)
        ...
        local output = ''
        local function write(str) output = output .. str end
        ...
        for module_name, mod in pairs(modules) do
            ...
            if not settings.minify then write('\n') end
            write(mod.content)
            if settings.minify then write(' ') end
            write('end\n')
            write('\n')
        end
...
__modules['src.cmd-parse'] = function(...)
    local path = require('src.path')
...
__modules['src.path'] = function(...)
    local path = {}
    function path.fill_path_template(path, file) return path:gsub('%?', file) end
    function path.dot_to_slash(module_name) return module_name:gsub('%.', '/') end
    function path.normalise_slashes(path) return path:gsub('\\', '/') end
    function path.open_module_file(include_paths, module_name)
        ...
        for _, include_path in ipairs(include_paths) do
            local full_path =
                path.normalise_slashes(path.fill_path_template(include_path, module_path))
            local file = io.open(full_path, 'r')
            if file then return file, full_path end
        end
        return nil
    end
...
__modules['src.animal-hash'] = function(...)
    ...
    local function generate_animal_hash()
        return select_animal(os.date('%H') + os.date('%M'))
            .. ' <3 '
            .. select_animal(os.date('%H') + os.date('%S'))
    end
